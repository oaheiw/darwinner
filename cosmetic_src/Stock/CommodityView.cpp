...
:ItemView(parent)
{
	...
	setHeaderData(col++, LOCAL8BITSTR(stReturnsStr));
	setHeaderData(col++, LOCAL8BITSTR(stActualBuysStr));
...
void CommodityView::addBusiness(Business& data)
{
	...
	addData(0, col++, data.getReturns());
	addData(0, col++, data.getActualBuys());
	addData(0, col++, data.cost());
	addData(0, col++, data.getAverageCost());
	addData(0, col++, data.stocks());
	addData(0, col++, data.discount());
	addData(0, col++, data.price());
	addData(0, col++, data.getAveragePrice());
