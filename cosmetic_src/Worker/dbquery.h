...
#define SELECT_STAFF_NOIMAGE "SELECT id, password, name, jobId, levelId, sex, baseSalary, status, cell, phone, address, descrption FROM staff  WHERE id > 10000"
...
#define UPDATA_JOB "UPDATE job SET name = '%1', profit = %2, baseSalary = %3, descrption = '%4' WHERE id = %5"
#define DELETE_JOB_BYID "DELETE FROM job WHERE id = %1"
#define CHECK_JOB_BYID "SELECT id FROM staff WHERE jobId = %1"

#define CREATE_LEVET_TABLE "CREATE TABLE level (id INTEGER PRIMARY KEY, name TEXT NOT NULL, profit SMALLINT DEFAULT 0, baseSalary INTEGER DEFAULT 0, descrption TEXT)"
#define INSERTINTO_LEVEL_TABLE "INSERT INTO level (name, profit, baseSalary, descrption) " "VALUES (:name, :profit, :baseSalary, :descrption)"
#define SELECT_LEVEL_ALL "SELECT * FROM level"
#define SELECT_LEVEL_BYID "SELECT * FROM level WHERE id = %1"
#define UPDATA_LEVEL "UPDATE level SET name = '%1', profit = %2, baseSalary = %3, descrption = '%4' WHERE id = %5"
